...
3. **Start talking**: Describe what you need - the system auto-routes to the right agent
...
This enables:
- Intent-based agent routing (auto-selects the right specialist)
- 17 expert agents for different development tasks
- Seamless handoffs between agents

Analyze every user message for intent and route to the appropriate agent.
```

### Cursor

Add to `.cursor/rules/bmad.md`:

```markdown
# BMAD Universal

Always reference bmad-universal/ORCHESTRATOR.md for agent routing.
Use intent detection to select appropriate specialist agents.
Announce agent switches with: 🤖 **[Agent Name]** ([Role]) is now active.
```

### Windsurf

Add to `.windsurf/rules/bmad.md`:

```markdown
# BMAD Universal Framework

Load bmad-universal/ORCHESTRATOR.md and follow its intent-based routing system.
Auto-select agents based on user intent, not keywords.
```

### Claude Code

Add to `.claude/settings.json` or mention in conversation:

```
Use the BMAD Universal framework in bmad-universal/ORCHESTRATOR.md
```

### Warp Terminal

Add to `~/.warp/rules/` or project `WARP.md`:
...
Simply say at the start of your conversation:
...
| `docs/` | Your project output folder |
